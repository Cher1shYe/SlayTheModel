...
def run_wave(*, mode: str, output: Path, game_dir: Path, ritsu_root: Path,
             ...
             max_decisions: int = 256, champion: Path | None = None) -> dict:
    ...
    manifest = {"format": "azcombat.wave.v1", "mode": mode, "gameVersion": "v0.111.0",
                "coverageCatalogSha256": sha256(catalog),
                "candidateOnnx": str(model), "candidateSha256": model_hash, "seeds": seeds,
                "encounters": encounters, "startTypes": list(STARTS), "budgetMilliseconds": budget_ms,
        ...
        for encounter in encounters:
            for start in STARTS:
                for policy in (("baseline", "candidate") if mode == "evaluate" else ("champion",)):
                    stem = f"{seed_index:03d}-{encounter}-{start}-{policy}"
                    ...
                    entry = {"seed": seed, "encounter": encounter, "startType": start, "policy": policy,
...
def main() -> int:
    ...
    result = run_wave(mode=args.mode, output=args.output, game_dir=args.game_dir,
                      ...
                      max_decisions=args.max_decisions, champion=args.champion)
