...
public static class Entry
{
    ...
    private static int _simulationRuntimeInitialized;

    public static void InitializeSimulationRuntime()
    {
        if (Interlocked.Exchange(ref _simulationRuntimeInitialized, 1) != 0) return;
        Logger = new CombatSolverLog(Path.Combine(OS.GetUserDataDir(), "logs", "CombatSolver.NativeMcts"));
        ModTypeDiscoveryHub.RegisterModAssembly(ModId, Assembly.GetExecutingAssembly());
        var patcher = RitsuLibFramework.CreatePatcher(ModId, "native-mcts-simulation", "Combat Solver simulation runtime");
