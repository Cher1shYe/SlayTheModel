...
try {
    ...
    if ($Action -eq 'Launch') {
        ...
        try {
            ...
            Start-Process `
                -FilePath $steamExe `
                -ArgumentList @('-applaunch', '2868840') `
                -WorkingDirectory (Split-Path $steamExe -Parent)

            Write-Host "Started through Steam runMode=$RunMode; combatPolicy=$CombatPolicy; outsideCombatPolicy=$OutsideCombatPolicy; captures=$ExportDir"
        } finally {
    ...
    if ($Action -eq 'Install') {
        ...
        $settings = @{
            ...
            # Persist the same three-dimensional launch contract used above.
            # RuntimeConfiguration reads this JSON when Steam does not forward
            # the temporary PowerShell environment to the game process.
            SLAY_THE_MODEL_RUN_MODE = $RunMode
            SLAY_THE_MODEL_COMBAT_POLICY = $CombatPolicy
            SLAY_THE_MODEL_OUTSIDE_COMBAT_POLICY = $OutsideCombatPolicy
            SLAY_THE_MODEL_LIVE_POLICY = $null
